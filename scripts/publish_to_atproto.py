...
class SpinglassATProto:
    ...
    def __init__(self, handle: str, password: str):
        self.client = Client()
        self.client.login(handle, password)  # login returns profile info :contentReference[oaicite:0]{index=0}

    def extract_article_metadata(self, html_path: str) -> Dict:
        """Extract title, excerpt, flags, and phase from HTML file"""
        with open(html_path, 'r', encoding='utf-8') as f:
            content = f.read()
        title = re.sub(r'[⟨⟩]', '', (re.search(r'<h1[^>]*>([^<]+)</h1>', content) or []).group(1) or "Untitled").strip()
        excerpt = html.unescape((re.search(r'<p>([^<]+)</p>', content) or []).group(1) or "")
        ...
        phase = "phaseα" if "phaseα" in html_path.lower() or "phasea" in html_path.lower() else \
                "phaseβ" if "phaseβ" in html_path.lower() or "phaseb" in html_path.lower() else \
                "phaseγ" if "phaseγ" in html_path.lower() or "phaseg" in html_path.lower() else "unknown"
        return {"title": title, "excerpt": excerpt, "has_glitch": has_glitch,
                "has_math": has_math, "phase": phase}

    def html_to_markdown(self, html_path: str) -> str:
        """Convert spinglasscore HTML to AT Protocol–compatible Markdown"""
        with open(html_path, 'r', encoding='utf-8') as f:
            content = f.read()
        # Strip body and styling, convert tags...
    ...
    def publish_article(self, html_path: str) -> Optional[str]:
        ...
        # New SDK requires a single Data object with repo, collection, record :contentReference[oaicite:1]{index=1}
        try:
    ...
    def publish_index_post(self, new_articles: list) -> Optional[str]:
        ...
        content = "\n\n".join(
            f"◈ **{self.extract_article_metadata(a)['title']}**"
...
def find_new_articles(last_run_file: str = ".atproto_last_run") -> list:
    ...
    new = [str(p) for p in Path(".").rglob("phase*/*.html")
...
def main():
    ...
    if args.file:
        ...
        for p in Path("phase*").rglob("*.html"):
